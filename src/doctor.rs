...
pub fn run() -> Result<()> {
    ...
    check_file("tasks.json", &zed.tasks, crate::install::is_zcc_task, "task entry labelled 'zcc-send'");
    check_file("keymap.json", &zed.keymap, crate::install::is_zcc_keymap_block, "keymap block with Cmd+L rendezvous");
...
fn check_binary_on_path() {
    ...
    match out {
        ...
        _ => {
            println!("not found");
            println!("    -> `brew install noahharborth/zcc/zcc` or add the binary to PATH");
        }
    }
}
...
fn tail_log() {
    let Ok(dir) = crate::logging::log_dir() else { return };
